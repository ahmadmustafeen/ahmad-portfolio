...
function ProjectRow({ project, index, mousePosition }) {
    ...
    return (
        <div className="max-w-6xl w-full flex flex-col lg:flex-row ml-18 items-center justify-between relative gap-8 lg:gap-12 py-10">
            <div ref={textRef} className={["w-full lg:w-[45%] text-theme-primary space-y-4 sm:space-y-5 z-10 reveal", textOrder].join(" ")}>
                ...
                <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-theme-primary leading-tight text-start">{project.title}</h3>
                <p className="text-theme-secondary leading-relaxed text-start text-sm sm:text-base">{project.description}</p>
                <p className="text-theme-secondary flex items-start text-sm sm:text-base">{project.stack}</p>
                <PrimaryButton>{project.cta.label}</PrimaryButton>
            </div>
...
export default function ProjectCard() {
    ...
    return (
        <section id="projects" className="w-full min-h-screen flex items-center justify-center   px-4 sm:px-6 lg:px-8 pt-12 relative">
